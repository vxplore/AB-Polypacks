<footer class="footer">
    <div class="ftrtop pt-5 pb-3 py-lg-5">
        <div class="container">
            <div class="row">
                <div class="col-lg-3 col-md-6 mb-4 mb-lg-0">
                    <a href="" class="ftrlogo">
                        <img alt="logo" width="80" src="<?=base_url('assets/client/images/footer-logo.png')?>">
                    </a>
                    <div class="my-3">AB Polypack is a reputable packaging company known for its innovative and high-quality packaging solutions. With a commitment to sustainability and customer satisfaction, AB Polypack specializes in providing a wide range of packaging materials and services tailored to meet the diverse needs of businesses across various industries.
                    </div>
                        <div class="socred d-flex gap-3 align-items-center">
                        <a href="https://www.facebook.com/people/A-B-Polypacks/100095289108636/" target="_blank">
                            <svg id="XMLID_834_" width="9" height="18" viewBox="0 0 10.5 20.786">
                                <path id="XMLID_835_" d="M77.038,11.07h2.278v9.38a.335.335,0,0,0,.335.335h3.863a.335.335,0,0,0,.335-.335V11.115H86.47a.335.335,0,0,0,.333-.3l.4-3.453a.335.335,0,0,0-.333-.374H83.851V4.826c0-.653.351-.983,1.044-.983h1.973a.335.335,0,0,0,.335-.335V.338A.335.335,0,0,0,86.867,0H84.149c-.019,0-.062,0-.125,0a5.209,5.209,0,0,0-3.406,1.284A3.572,3.572,0,0,0,79.43,4.459V6.991H77.038a.335.335,0,0,0-.335.335v3.409A.335.335,0,0,0,77.038,11.07Z" transform="translate(-76.703 0)" fill="#fff"></path>
                            </svg>
                        </a>
                        
                        <a href="https://www.instagram.com/ab.polypacks/" target="_blank">
                            <svg width="17" height="17" viewBox="0 0 21 22">
                                <g id="instagram-svgrepo-com_1_" data-name="instagram-svgrepo-com (1)" transform="translate(0.569 1.159)">
                                    <rect id="Rectangle_213" data-name="Rectangle 213" width="19" height="20" rx="5" transform="translate(0.431 -0.159)" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></rect>
                                    <path id="Path_102" data-name="Path 102" d="M15.874,11.316A3.937,3.937,0,1,1,12.557,8,3.937,3.937,0,0,1,15.874,11.316Z" transform="translate(-2.096 -2.234)" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path>
                                    <line id="Line_30" data-name="Line 30" x2="0.01" transform="translate(15.255 4.289)" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></line>
                                </g>
                            </svg>
                        </a>
                        <a href="https://www.linkedin.com/company/a-b-polypacks" target="_blank">
                            <svg id="Bold" enable-background="new 0 0 24 24" fill="#fff" height="18" viewBox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg"><path d="m23.994 24v-.001h.006v-8.802c0-4.306-.927-7.623-5.961-7.623-2.42 0-4.044 1.328-4.707 2.587h-.07v-2.185h-4.773v16.023h4.97v-7.934c0-2.089.396-4.109 2.983-4.109 2.549 0 2.587 2.384 2.587 4.243v7.801z"></path><path d="m.396 7.977h4.976v16.023h-4.976z"></path><path d="m2.882 0c-1.591 0-2.882 1.291-2.882 2.882s1.291 2.909 2.882 2.909 2.882-1.318 2.882-2.909c-.001-1.591-1.292-2.882-2.882-2.882z"></path></svg>
                        </a>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mb-4 mb-lg-0">
                    <h3>Quick Links</h3>
                    <ul class="ftrmnu m-0 p-0">
                        <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url()?>">Home</a>
                        </li>
                        <!-- <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('about')?>">About Us</a>
                        </li> -->
                        <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('products')?>">Products</a>
                        </li>
                        <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('carrer')?>">Career</a>
                        </li>
                        <!-- <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('client')?>">Client</a>
                        </li>
                        <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('blog')?>">Blog</a>
                        </li> -->
                        <li>
                            <svg width="12.483" height="8.324" viewBox="0 0 12.483 8.324">
                                <path id="right-arrow-1-svgrepo-com" d="M12.483,94.962,7.283,90.8v3.121H0V96H7.283v3.121Z" transform="translate(0 -90.8)" fill="#fff"/>
                            </svg>
                            <a href="<?=base_url('contact-us')?>">Contact Us</a>
                        </li>
                    </ul>
                </div>

                <?php if (!empty($office_contact_info)) { ?>
                    <div class="col-lg-3 col-md-6 mb-4 mb-lg-0">
                        <h3>Office Contact Info.</h3>
                        <ul class="ftrmnu m-0 p-0">
                            <?=(!empty($office_contact_info->company_name)) ? "<li>".$office_contact_info->company_name."</li>" : ""?>

                            <?php if (!empty($office_contact_info->phone)) {
                            $phone = $office_contact_info->phone; ?>
                                <li>Call: <a href="tel:<?=$phone?>"><?=$phone?></a></li>
                            <?php } ?>

                            <?php if (!empty($office_contact_info->email)) {
                            $email = $office_contact_info->email; ?>
                                <li>Email: <a href="mailto:<?=$email?>"><?=$email?></a></li>
                            <?php } ?>

                            <?php if (!empty($office_contact_info->address)) {
                            $address = $office_contact_info->address; ?>
                                <li class="align-items-start">Location: <a href="https://goo.gl/maps/SDVgjRfRwfnm351bA" target="_blank"><?=$office_contact_info->address?></a></li>
                            <?php } ?>
                        </ul>
                    </div>
                <?php } ?>
                
                <div class="col-lg-3 col-md-6 mb-4 mb-lg-0">
                    <h3>Subscribe Newsletter</h3>
                    <p>Subscribe to our Newsletter and get latest news.</p>
                    <form class="d-block" action="#" method="post">
                        <div class="subsfrm d-flex align-items-center p-1 bg-white rounded-pill">
                            <input type="email" class="form-control border-0" placeholder="Email Address" required>
                            <button class="subsbtn" type="submit">Subscribe</button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <div class="ftrbtm text-white text-center">
        <div class="container">
            <div class="ftrbtminr py-3 border-top">
                <div class="row">
                    <div class="col-md-6 text-md-start text-center">
                        Copyright 2023 A.B. Polypacks PVT. LTD. All Rights Reserved.
                    </div>
                    <div class="col-md-6 mt-3 mt-md-0">
                        <div class="d-flex gap-1 justify-content-center justify-content-md-end">
                            <a href="<?=base_url('privacy-policy')?>">Privacy Policy</a>
                            <span>|</span>
                            <a href="<?=base_url('terms-and-conditions')?>">Terms & Conditions</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>