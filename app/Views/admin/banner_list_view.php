        ...
        <div class="card-body">
              ...
              <table id="banners_listing_table" class="table table-hover table-bordered">
                ...
                <tbody>
                  <?php foreach ($list_of_banners as $i => $banner_details) { ?>
                    <tr data-banner-id="<?=$banner_details->banner_id?>">
                      <th scope="row">
                      ...
                      <td scope="row">
                        <div class="d-flex align-items-center">
                            ...
                            <a class="deletebtn bgdelete ms-2" href="javascript:delete_banner('<?=$banner_details->banner_id?>')"><i class="bi bi-trash3"></i></a>
                        </div>
                      </td>
                    </tr>
                  <?php }?>
          ...
          <?php } else { ?>
            <div class="empty-container">
              <h5 class="empty-container-note">No Banners Found!</h5>
            </div>
          <?php } ?>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-body text-center">
          ...
          <h4>Are you sure ?</h4>
          <p class="pb-0 mb-0">Do you really want to delete this banner ?</p>
          <form id="banner_delete_form">
            <input type="hidden" name="banner_id" id="deletable_banner_id">
          </form>
        </div>
...
<script>
  ...
  $("#banners_listing_table tbody").dragsort({
    ...
    dragEnd: function() {
      change_banner_appearing_order();
    }
  });

  function change_banner_appearing_order() {
    let rows = $("#banners_listing_table tbody tr");
    let banner_appearing_orders = [];
    for (let i=0; i<rows.length; i++) {
      let row = rows[i];
      let appearing_order_details = {
        banner_id: row.dataset.bannerId,
        appearing_order: parseInt(i+1)
      };
      banner_appearing_orders.push(appearing_order_details);
    }

    $.ajax({
      url: "<?=base_url('admin/banners/change-banner-appearing-order')?>",
      type: "POST",
      data: JSON.stringify(banner_appearing_orders),
